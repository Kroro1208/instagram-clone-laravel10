    ...
    <footer>
        ...
        @if ($post->allow_commenting)
                ...
                <ul class="my-2">
                    @foreach($post->comments()->where('user_id', auth()->id())->get() as $comment)
                        <li class="grid grid-cols-12 text-sm items-center">
                            ...
                            <button class="col-span-1 mb-auto flex justify-end">
            ...
            <form
                ...
                @csrf
                <input x-model="body"ype="text" placeholder="コメントを追加..." class="border-0 col-span-10 placeholder:text-sm outline-none focus:outline-none px-0 rounded-lg hover:ring-0 focus:ring-0">
                <div class="col-span-1 ml-auto flex justify-end text-right">
                    <button type="submit" x-cloak x-show="body.ngth>0" class="text-sm font-semibold flex justify-end text-blue-500">
                        投稿する
                    </button>
                </div>
