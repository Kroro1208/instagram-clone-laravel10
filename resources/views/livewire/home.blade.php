...
>
    ...
    <header class="md:hidden sticky top-0 bg-white">
        <div class="grid grid-cols-12 gap-2 items-center">
            <div class="col-span-3">
                <img src="{{asset('assets/logo.png')}}" alt="logo" class="h-12 w-full max-w-lg">
            </div>
            <div class="col-span-8 flex justify-center px-2">
                <input type="text" placeholder="検索" class="border-0 outline-none w-full focus:outline-none bg-gray-100 rounded-lg focus:ring-0 hover:focus:ring-0">
            </div>
    ...
    <main class="grid lg:grid-cols-12 gap-8 md:mt-10">
        <aside class="lg:col-span-8 overflow-hidden">
            {{--Stories--}}
            <section>
                <ul class="flex overflow-x-auto scrollbar-hide md:scrollbar-default items-center gap-2">
                    @for($i=0; $i < 10; $i ++) <li class="flex flex-col justify-center w-20 gap-1 p-2">
                        <x-avatar story src="http://source.unsplash.com/500x500?face-{{$i}}" class="h-14 w-14" />
        ...
        <aside class="lg:col-span-4  hidden lg:block p-4">
            <div class="flex items-center gap-2">
                <x-avatar src="http://source.unsplash.com/500x500?face" class="w-12 h-12" />
                <h4 class="font-medium">{{fake()->name}}</h4>
            </div>

            {{--Suggestions--}}
            <section class="mt-4">
                <h4 class="font-bold text-gray-700/95">おススメユーザー</h4>
                <ul class="my-2 space-y-3">
                    @for($i=0; $i<5; $i++) <li class="flex items-center gap-3">
                        <x-avatar src="http://source.unsplash.com/500x500?face-{{$i}}" class="h-12 w-12" />
